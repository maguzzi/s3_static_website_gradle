...
public class App {
    ...
    public App(String[] args, CloudFormationClient cloudFormationClient, S3Client s3Client) throws Exception {
        ...
        switch (command) {
            ...
            case "DISTRIBUTION": {
                ...
                File outputPath = new File(outputFromZipArtifactCommand.get(ZipArtifactCommand.ARTIFACT_COMPRESSED_PATH).getValue());
                ...
                Map<String,OutputEntry> uploadLambdaArtifactResult = uploadLambdaNestedStackSourceCodeFileToBucket.execute();
                mapToString(uploadLambdaArtifactResult);

                Map<String,OutputEntry> uploadLambdaTemplateResult = uploadLambdaNestedStackTemplateFileToBucket.execute();
                ...
                distributionInput.put(S3_BUCKET_NAME_PARAMETER,String.format("%s-%s-%s",S3_STATIC_WEBSITE_BUCKET,CommandUtil.dateToSecond(),environment));
                distributionInput.put(BOOTSTRAP_ARTIFACT_S3_BUCKET_NAME_EXPORT_NAME,outputFromStackResult.get(COMPILED_TEMPLATE_BUCKET_KEY).getExportName());
    ...
    private void mapToString(Map<String,OutputEntry> result) {
        result.entrySet().forEach(e -> logger.info("{} -> {} ({})",e.getKey(),e.getValue().getValue(),e.getValue().getExportName()));
    }
